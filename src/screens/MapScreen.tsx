...
import {
  ...
  Animated, Image, PanResponder, Dimensions, Modal, ScrollView,
} from 'react-native';
...
import { API_BASE, getNearbySellers, setSellerLocation, getImageUrl, getProducts, toggleFollow, getFollowing, getFollowerCount } from '../api';
...
async function loadMapHtml(maptilerKey?: string | null): Promise<string> {
  const cacheKey = maptilerKey || 'nokey';
  if (_cachedUri && _lastKey === cacheKey) return _cachedUri;
  const { cacheDirectory, writeAsStringAsync } = await import('expo-file-system/legacy');
  const tileUrl = maptilerKey
    ? `https://api.maptiler.com/tiles/vulkan/{z}/{x}/{y}.png?key=${maptilerKey}`
...
export default function MapScreen() {
  ...
  const dbg = useCallback((msg: string) => {
    ...
    const entry = `[${ts}] ${msg}`;
    debugLogsRef.current = [...debugLogsRef.current.slice(-80), entry];
    setDebugLogs([...debugLogsRef.current]);
  }, []);

  const sendReport = useCallback(async (extra?: string) => {
    const allLogs = [...debugLogsRef.current, extra ? `[CRASH] ${extra}` : ''];
    try {
      await fetch(`${API_BASE}/debug/map-report`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ logs: allLogs, platform: Platform.OS, timestamp: new Date().toISOString() }),
      });
    } catch {}
  }, []);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        try {
          const cfgRes = await fetch(`${API_BASE}/map-config`);
          const cfg = await cfgRes.json();
          maptilerKey = cfg.maptilerKey;
          dbg('MapTiler key: ' + (maptilerKey ? maptilerKey.slice(0, 8) + '...' : 'null'));
        } catch (e: any) {
          dbg('map-config fetch failed: ' + (e?.message || String(e)));
        }
        dbg('Loading map HTML...');
        const html = await loadMapHtml(maptilerKey);
        dbg('HTML loaded OK');
        setHtmlContent(html);
      } catch (err: any) {
  ...
  return (
    <View style={styles.container}>
      ...
      {showDebug && (
        <View style={[styles.debugOverlay, { top: insets.top + 50 }]} pointerEvents="box-none">
          <TouchableOpacity onPress={() => setShowDebug(!showDebug)} style={styles.debugToggle}>
            <MaterialCommunityIcons name="bug" size={14} color={COLORS.text2} />
          </TouchableOpacity>
          ...
          <TouchableOpacity onPress={() => sendReport()} style={styles.debugSendBtn}>
            <Text style={styles.debugSendText}>Send Report</Text>
          </TouchableOpacity>
        </View>
      )}
      {!showDebug && (
        <TouchableOpacity onPress={() => setShowDebug(true)} style={[styles.debugToggle, { top: insets.top + 50, right: SPACING.lg }]}>
          <MaterialCommunityIcons name="bug" size={18} color={COLORS.text2} />
        </TouchableOpacity>
      )}
...
const styles = StyleSheet.create({
  ...
  debugToggle: {
    position: 'absolute', right: SPACING.lg, width: 30, height: 30,
    borderRadius: 15, backgroundColor: 'rgba(0,0,0,0.7)',
    alignItems: 'center', justifyContent: 'center', zIndex: 99,
  },
  debugScroll: { flex: 1 },
  debugText: { color: '#8B949E', fontSize: 10, fontFamily: 'Courier', lineHeight: 14 },
  debugSendBtn: { marginTop: 4, paddingVertical: 4, alignItems: 'center' },
  debugSendText: { color: COLORS.coral, fontSize: 11, fontWeight: '700' },
});
