...
export default function AddListingScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (isNaN(priceNum) || priceNum < 100) {
      toast.error(t('addListing.missingInfo'), 'Minimum price is 100 G');
      return;
    }
    if (priceNum > 99999) {
      toast.error(t('addListing.missingInfo'), 'Maximum price is 99,999 G');
      return;
    }
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {atListingLimit ? (
        ...
        <>
          ...
          <TextInput style={styles.input} placeholder={`${t('addListing.price')} (100-99,999 G)`} placeholderTextColor={COLORS.text2} value={price} onChangeText={(v) => { const num = v.replace(/[^0-9]/g, ''); if (!num || Number(num) <= 99999) setPrice(num); }} keyboardType="numeric" accessibilityLabel="price" maxLength={5} />
